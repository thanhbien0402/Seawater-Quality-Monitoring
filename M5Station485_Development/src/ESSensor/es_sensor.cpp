...
void initRS485(uart_port_t uart_num, int baudrate, int rxPin, int txPin)
{
    ...
    // RS485Serial.setRxTimeout(rxTimeout); // 10ms timeout
...
void turnPumpRelay(bool state) {
    ...
    if (state) {
        ...
        // Serial.println("Turning pump relay ON");
    } else {
        // Command to turn off relay No. 1 (manual mode)
        command[0] = 0xFF;
        command[1] = 0x05;
        command[2] = 0x00;
        command[3] = 0x00;
        ...
        // Serial.println("Turning pump relay OFF");
    }

    const int MAX_RETRIES = 3;
    // Retry up to 3 times if the response does not match the sent command
    for (int retry = 0; retry < MAX_RETRIES; retry++) {
        ...
        if (length_rx > 0) {
            ...
            if (match) {
                ...
                success = true;
                break;  // Exit the loop since the response matched the command
            } else {
        ...
        vTaskDelay(250 / portTICK_RATE_MS); // Delay between MAX_RETRIES
    }

    if (!success) {
        ESP_LOGE(TAG, "Failed to turn relay on/off after 3 attempts.");
    }    
}

bool readPumpStatus()
{
    uint8_t command[8] = {0xFF, 0x01, 0x00, 0x00, 0x00, 0x08, 0x28, 0x12};
    uint8_t response_frame[8];
    bool status = false;
    ...
    for (int retry = 1; retry <= MAX_RETRIES; retry++) {
        ...
        {
            ESP_LOGW(TAG, "No response for %s, retrying %d/%d", "pump relay status", retry, MAX_RETRIES);
            uart_flush(UART_NUM_2);
            vTaskDelay(1500 / portTICK_RATE_MS);
        }
    }
    ESP_LOGE(TAG, "Failed to read %s after %d attempts", "pump relay status", MAX_RETRIES);    
    vTaskDelay(250 / portTICK_RATE_MS); // Delay between MAX_RETRIES
    return status;
}